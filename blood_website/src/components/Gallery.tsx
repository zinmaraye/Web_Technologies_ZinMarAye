    ...
    axios.get('http://localhost:8000/api/donation/gallery/list')
      .then((response) => {
        ...
        if (response.data.status) {
          ...
          const images = response.data.data.map((item: { image: string; }) => JSON.parse(item.image));
      ...
      .catch((error) => {
        // Check if error.response exists or fallback to error.message
            ...
            galleryData.map((image, index) => (
              <div key={index} className="relative overflow-hidden rounded-lg aspect-square">
                <img
                  ...
                  src={`http://localhost:8000/storage/${image}`} // Assuming images are in 'storage' folder
